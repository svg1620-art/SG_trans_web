...
def run_job(job_id, tmp_path, filename, account, user_prompt, mode, manager_name):
    try:
        ...
        if mode == "sales_club":
            summary, metrics = run_analysis(SALES_CLUB_SCRIPT, "Ты аналитик отдела продаж B2B SaaS. Заполняй все поля максимально конкретно. В конце ОБЯЗАТЕЛЬНО выведи JSON. Отвечай на русском.", transcript, user_prompt)
        elif mode == "sales":
            summary, metrics = run_analysis(SALES_SCRIPT, "Ты аналитик отдела продаж B2B SaaS. Заполняй все поля максимально конкретно. В конце ОБЯЗАТЕЛЬНО выведи JSON. Отвечай на русском.", transcript, user_prompt)
        elif mode == "renewal":
            summary, metrics = run_analysis(RENEWAL_SCRIPT, "Ты аналитик клиентского сервиса B2B SaaS. Заполняй все поля максимально конкретно. В конце ОБЯЗАТЕЛЬНО выведи JSON. Отвечай на русском.", transcript, user_prompt)
        else:
...
def dashboard(dash_type):
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("SELECT * FROM calls WHERE account=%s ORDER BY created_at DESC", (session["user"],))
