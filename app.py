...
ОЦЕНКИ В JSON (обязательно в конце, строго без markdown блоков, просто JSON):
{"scores":{"overall":0,"qualification":0,"presentation":0,"objections":0,"closing":0},"errors":[],"manager_tasks":[]}

Где errors — список из 1-3 главных ошибок одной фразой, manager_tasks — список из 2-3 конкретных заданий на отработку."""
...
def extract_json_from_text(text):
    ...
    try:
        match = re.search(r'\{[^{}]*"scores"[^{}]*\}', text_clean, re.DOTALL)
        if match:
...
def run_job(job_id, tmp_path, filename, account, user_prompt, mode, manager_name):
    try:
        ...
        if mode == "sales_club":
            summary, metrics = run_analysis(SALES_CLUB_SCRIPT, "Ты аналитик отдела продаж B2B SaaS. Заполняй все поля максимально конкретно. В конце выведи JSON с оценками БЕЗ markdown блоков. Отвечай на русском.", transcript, user_prompt)
        elif mode == "sales":
            summary, metrics = run_analysis(SALES_SCRIPT, "Ты аналитик отдела продаж B2B SaaS. Заполняй все поля максимально конкретно. В конце выведи JSON с оценками БЕЗ markdown блоков. Отвечай на русском.", transcript, user_prompt)
        elif mode == "renewal":
            summary, metrics = run_analysis(RENEWAL_SCRIPT, "Ты аналитик клиентского сервиса B2B SaaS. Заполняй все поля максимально конкретно. В конце выведи JSON с оценками БЕЗ markdown блоков. Отвечай на русском.", transcript, user_prompt)
        else:
