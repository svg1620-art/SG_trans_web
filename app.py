...
# Simple user store — add users here or via env
...
<div id="login-screen">
  <div class="login-card">
    ...
    <div class="field">
      <label>Пароль</label>
      <input type="password" id="password" placeholder="Введите пароль" onkeydown="if(event.key==='Enter')doLogin()">
    </div>
...
<div id="app-screen">
  ...
  <div class="main">
    <div class="upload-section">
      ...
      <button class="btn" id="transcribe-btn" onclick="doTranscribe()" disabled>
      ...
      <div class="result-card" id="result-card">
        <div class="result-header">
          ...
          <button class="download-btn" id="download-btn" onclick="doDownload()">⬇ Скачать TXT</button>
        </div>
        ...
        <div class="tab-content" id="tab-transcript">
          <div class="result-text" id="result-transcript"></div>
        </div>
      </div>
    ...
    <div class="history-section">
      ...
      <div id="history-list">
        <div class="history-empty">Запросов пока нет</div>
      </div>
    </div>
  </div>
</div>
...
async function doTranscribe() {
  ...
  const ticker = setInterval(() => {
    ...
    const status = p < 40 ? 'Загружаю файл…' : p < 65 ? 'Расшифровываю аудио…' : 'Делаю саммери…';
...
function doDownload() {
  if (currentEntryId) window.open('/api/download/' + currentEntryId);
}
...
async function loadHistory() {
  ...
  el.innerHTML = d.history.map(h => `
    <div class="history-item" onclick="downloadEntry('${h.id}')">
